...
const int SOUND_PIN = 5;    // Sound detection with microphone - not active
...
//when setting up Neopixel library, we tell how many 
...
//colour for default
void WarmYellow() {
  strip.fill(strip.Color(255, 150, 0));
  strip.setBrightness (10);
  strip.show();
}
//Colour for meowing
void colourVibrant() {
  strip.fill(strip.Color(252, 3, 244));
  strip.setBrightness (10);
...
//colour for purring
void colourBlue() {
  strip.fill(strip.Color(3, 127, 252));
  strip.setBrightness (10);
  strip.show();
}
...
if (currentTouchState = digitalRead(SENSOR_PIN)) {
